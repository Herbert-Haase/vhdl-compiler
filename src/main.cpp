...
void printExprDot(ExprNode *expr, int &nodeId, int parentId) {
  ...
  if (auto *id = dynamic_cast<IdExpr *>(expr)) {
    ...
    for (auto &op : a->operands)
      printExprDot(op.get(), nodeId, currentId);
...
void printASTDot(ASTNode *node, int &nodeId, int parentId = -1) {
  ...
  if (auto *start = dynamic_cast<StartRule *>(node)) {
    ...
    for (auto &lib : start->libs)
      printASTDot(lib.get(), nodeId, currentId);
    for (auto &use : start->uses)
      printASTDot(use.get(), nodeId, currentId);
    for (auto &ent : start->entities)
      printASTDot(ent.get(), nodeId, currentId);
    for (auto &arch : start->arches)
      printASTDot(arch.get(), nodeId, currentId);
    return;
  } else if (auto *ent = dynamic_cast<Entity *>(node)) {
    label = "Entity: " + ent->name;
    color = "lightgreen";
    if (ent->port)
      printASTDot(ent->port.get(), nodeId, currentId);
  } else if (auto *port = dynamic_cast<Port *>(node)) {
    label = "PORT";
    for (auto &sig : port->signals)
      printASTDot(sig.get(), nodeId, currentId);
  } else if (auto *pd = dynamic_cast<PortDecl *>(node)) {
    label =
        (pd->in ? "[IN] " : "[OUT] ") + joinNames(pd->names) + " : " + pd->type;
    color = "lightyellow";
    ...
    if (parentId != -1)
      std::cout << "  n" << parentId << " -> n" << currentId << ";\n";
    for (auto &sig : arch->signals)
      printASTDot(sig.get(), nodeId, currentId);
    for (auto &stmt : arch->statements)
      printASTDot(stmt.get(), nodeId, currentId);
    return;
    ...
    if (parentId != -1)
      std::cout << "  n" << parentId << " -> n" << currentId << ";\n";
    if (stmt->expr)
      printExprDot(stmt->expr.get(), nodeId, currentId);
    return;
  ...
  std::cout << "  n" << currentId << " [label=\"" << label << "\", fillcolor=\""
            << color << "\", style=filled];\n";
  if (parentId != -1)
    std::cout << "  n" << parentId << " -> n" << currentId << ";\n";
}
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::cerr << "Usage: " << argv[0] << " <file.vhd>\n"
              << "Or for Interpreter: " << argv[0]
              << " <file.vhd> a=0 b=1 c=1\n";
    return 1;
  }
  ...
  if (!checker.check(static_cast<StartRule &>(*builder.root)))
    return 1;

  if (argc > 2) {
    Interpreter interpreter;
    if (interpreter.filloutArgs(argc, argv) > 0) 
      return 1;
    interpreter.output(static_cast<StartRule &>(*builder.root));
  } else {
